...
#                                                  #
#                                                  #
#    WK 6:  Visualising High Dimensional Data      #
#                                                  #
#                                                  #
####################################################
...
# If you get a question saying " Do you want to install from sources the package which needs compliation?"
# Say yes
...
# Some packages aren't in the main database of approved R packages, but are still fantastic, well-supported,
# and really useful. Thoes can't be downloaded with `install.packages()`. 
# For these, you need to use `devtools::install_github("")` to install from github.

# Install the following and load their libraries
# If they ask you to update other packages, select 'None'
...
# Create synthetic data using the code below 
set.seed(1)
...
# 6. + theme_ipsum() :
# 7. + theme(plot.title = element_text(size = )) :
...
# plto 1 - add geom_alluvial(), use `fill = admit`` to add colour, and `width` to set the width of the layers (stratum)
alluvial_1 <- alluvial_0 + geom_alluvium(aes(fill = Admit), width = 1/12)
alluvial_1

#plot 2 - add axis bars and label them. `width` sets the width of the verticle bars
...
# Plot 3 - change labels on the x axis
...
# Final plot 
ggplot(UCB, aes(y = Freq, axis1 = Gender, axis2 = Dept)) + 
