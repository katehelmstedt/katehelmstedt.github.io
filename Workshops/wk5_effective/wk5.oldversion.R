...
## -- TASK: recreate the plot above using a different theme. Type `theme_` and select tab, a drop down list of themes will appear. 
...
# tell R which variable to use for colours in either `aes()` or in the geom() ... then customise the colours using one of the `scale_colour_...()` functions below. 

# Categorical variables use
+ scale_fill_manual()
+ scale_fill_brewer() # must insert `palette = ` 

# Continuous variables use
+ scale_colour_gradient()  # sequential
+ scale_colour_gradient2() # diverging
 
# Plot examples
# add colour using aes() within ggplot()
ggplot(iris, aes(Species, Sepal.Length), colour = Species) +
  geom_point()   

# add colour using aes() within geom()
ggplot(iris, aes(Species, Sepal.Length)) + 
  geom_boxplot(aes(fill = Species))   

# change colour by adding + scale_fill_manual
ggplot(iris, aes(Species, Sepal.Length)) + 
  geom_boxplot(aes(fill = Species)) + 
  scale_fill_manual(values = c("#00AFBB", "#E7B800", "#FC4E07"))
  
# change colour using a predefined color palette.
ggplot(iris, aes(Species, Sepal.Length)) + 
  geom_boxplot(aes(fill = Species)) +
...
############# add example of continous gradient###########
